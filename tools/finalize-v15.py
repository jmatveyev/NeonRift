...
replace(p,'    begin(){this.batches.clear();}',"    ready(force=false){const g=this.gl;if(!this.fence)return true;if(force)g.finish();const status=g.clientWaitSync(this.fence,0,0);if(status===g.TIMEOUT_EXPIRED)return false;g.deleteSync(this.fence);this.fence=null;return true;}\n    begin(){this.batches.clear();}")
replace(p,"this.uniform('impacts[0]',4,imp);this.quad();","this.uniform('impacts[0]',4,imp);this.quad();if(this.fence)g.deleteSync(this.fence);this.fence=g.fenceSync(g.SYNC_GPU_COMMANDS_COMPLETE,0);g.flush();")
p=SOURCE/'models.js'
...
replace(p,'function nr15Render(){if(!nr15State.engine',"function nr15Render(force=false){if(nr15State.mode==='webgl2'&&nr15State.engine&&!nr15State.engine.ready(force))return;const now=performance.now();const key=[state,W,H,selectedRig,nr15State.quality].join('/');if(!force&&state!=='playing'&&state!=='home'&&nr15State.staticKey===key)return;if(!force&&coarse&&key===nr15State.staticKey&&now-(nr15State.lastPaint||0)<32)return;nr15State.lastPaint=now;nr15State.staticKey=key;if(!nr15State.engine")
replace(p,'},render:nr15Render,loseContext()','},render:()=>nr15Render(true),loseContext()')
p=SOURCE/'build.py'
focus_old="function focusButton(id) { requestAnimationFrame(() => { if (!$(id).hidden && $(id).offsetParent !== null) $(id).focus({ preventScroll: true }); }); }"
focus_new="function focusButton(id) { queueMicrotask(() => { if (!$(id).hidden && $(id).offsetParent !== null) $(id).focus({ preventScroll: true }); }); }"
anchor="once('</style>',(HERE/'presentation.css').read_text()+'\\n</style>')"
replace(p,anchor,"once("+repr(focus_old)+","+repr(focus_new)+")\n"+anchor)
...
print('Reviewed shadow, focus, and presentation-scheduling fixes applied.')
